...
#'   species-level classifications in the output (species = 'all'; default),
#'   or only those generated by exact matching or near-neighbor searching
#'   (species = 'ping'). In the latter case, non-matched species are returned
#'   at genus level.
#'   Alternatively, if species = 'none', all species-level classifications
#'   are rounded to genus level.
...
classify <- function(x, tree, threshold = 0.9, decay = TRUE, ping = TRUE,
                     ...
                     species = "all", tabulize = FALSE, metadata = FALSE, cores = 1){
  ...
  if(species %in% c("ping", "none")){
    lowrank <- db$rank == tail(ranks, 1)
    dblr <- db$name[lowrank] # lowest rank in db
    hasspp <- sum(grepl("[[:lower:]] [[:lower:]]", dblr))/length(dblr) > 0.5
    if(hasspp){
      whichspp <- res$taxID %in% db$taxID[lowrank]
      if(species == "ping") whichspp[is.na(res$score)] <- FALSE # leave pinged spps as is
      if(sum(whichspp) > 0){
