...
#' @return an integer vector of cluster membership with values between 1 and
...
otu <- function(x, k = 5, threshold = 0.97, ...){
  ...
  kcounts <- round(phylogram::kcount(x, k = k, named = FALSE))
  tree <- 1
  attr(tree, "leaf") <- TRUE
  ...
  otu2 <- function(node, d, threshold, ...){
    if(!is.list(node) & length(attr(node, "sequences")) > 1){
      ...
      errfun <- function(er) return(NULL)
      km <- if(nrow(seqs) > 2){
