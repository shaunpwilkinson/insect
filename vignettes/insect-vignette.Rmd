...
raw sequence data input to tabular output. At any stage of the workflow,
users can export their partially-processed data for input into other programs
using the functions `writeFASTA` and `writeFASTQ`.
...
in Timor-Leste, amplified using the COI barcoding primers 
...
in either direction, trimming the primers off those that do, 
and finally orientating all sequences in the 5' -> 3' direction.
...
since the function uses the computationally intensive Needleman–Wunsch 
...
Again this may take a minute or so to process, 
...
However, note that over-specifying the number of cores can have an adverse
effect due to the extra time taken to initialize the cluster 
(in this small example the optimal number of cores is ~ 2, 
beyond which the addition of extra cores becomes counter-productive).
