...
without a confidence score. To stay on the safe side, we will set
...
table that includes one column of sequence counts for each sample.
    ...
    longDF <- classify(x, classifier_mo, cores = 2, tabulize = FALSE, ping = 1)
...
the sequences as follows:
    ...
    ## assign taxon IDs
    longDF <- classify(x_nt, classifier_nt, cores = 1, tabulize = FALSE, ping = 1)
...
This has improved the resolution to some extent, but we can recall even
more taxa to class level and below by using an amino acid sequence
...
package, and converted back to binary form object using `ape::as.AAbin`.
    ...
    keeps <- sapply(x_aa, function(v) !any(v == as.raw(42)))
    ...
    ## assign taxon IDs
    longDF <- classify(x_aa, classifier_aa, cores = 1, tabulize = FALSE, ping = 1)
